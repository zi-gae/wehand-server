...
import { matchCreateSchema, matchJoinSchema, matchFilterSchema, uuidSchema } from '../utils/validation';
import { formatDate, formatPrice, calculateDistance, sanitizeString } from '../utils/helpers';
...
export const matchController = {
  ...
  getMatches: asyncHandler(async (req: Request, res: Response) => {
    ...
    const { page, limit, search, region, game_type, date, ntrp_min, ntrp_max, experience_min, experience_max, sort } = filters;
    ...
    if (experience_min || experience_max) {
      if (experience_min) query = query.gte('recruit_experience_max', experience_min);
      if (experience_max) query = query.lte('recruit_experience_min', experience_max);
    }
    ...
    return ResponseHelper.successWithPagination(res, formattedMatches, pagination);
  }),
  ...
  getMatchDetail: asyncHandler(async (req: Request, res: Response) => {
    ...
    const confirmedParticipants = participants?.map((p: any) => ({
      id: p.users.id,
      name: p.users.name,
      ntrp: p.users.ntrp?.toString() || '미설정',
      experience: p.users.experience_years ? `${p.users.experience_years}년` : '미설정',
      isHost: p.is_host
    })) || [];
    ...
    const formattedMatch = {
      ...
      hostExperience: match.host_experience ? `${match.host_experience}년` : '미설정',
      description: match.description || '',
      rules: match.rules || [],
  ...
  joinMatch: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (match.host_id === userId) {
      throw new ApiError(400, '자신이 호스트인 매치에는 참가 신청할 수 없습니다', 'CANNOT_JOIN_OWN_MATCH');
    }

    if (match.status !== 'recruiting') {
      throw new ApiError(400, '모집 중인 매치가 아닙니다', 'MATCH_NOT_RECRUITING');
    }
    ...
    if (existingParticipation) {
      ...
      throw new ApiError(409, statusMessage[existingParticipation.status] || '이미 참가 신청한 매치입니다', 'ALREADY_APPLIED');
    }
    ...
    if (insertError) {
      ...
      throw new ApiError(500, '참가 신청 처리 중 오류가 발생했습니다', 'JOIN_ERROR');
    }
  ...
  createMatch: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (venueError || !venue) {
      throw new ApiError(404, '선택한 테니스장을 찾을 수 없습니다', 'VENUE_NOT_FOUND');
    }
    ...
    const { data: newMatch, error: insertError } = await supabase
      ...
      .insert({
        ...
        description: matchData.description ? sanitizeString(matchData.description) : null,
        host_id: userId,
    ...
    if (insertError) {
      ...
      throw new ApiError(500, '매치 생성 중 오류가 발생했습니다', 'MATCH_CREATION_ERROR');
    }
  ...
  shareMatch: asyncHandler(async (req: Request, res: Response) => {
    ...
    const shareData = {
      ...
      title: match.title,
      description: `${formatGameType(match.game_type)} • ${match.price ? formatPrice(match.price) : '무료'}`
    };
  ...
  bookmarkMatch: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (error) {
      throw new ApiError(500, '북마크 저장 중 오류가 발생했습니다', 'BOOKMARK_ERROR');
    }
  ...
  unbookmarkMatch: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (error) {
      throw new ApiError(500, '북마크 해제 중 오류가 발생했습니다', 'UNBOOKMARK_ERROR');
    }
  ...
  createMatchChat: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!participation) {
      throw new ApiError(403, '매치 참가자만 채팅방을 생성할 수 있습니다', 'NOT_MATCH_PARTICIPANT');
    }
    ...
    if (existingChat) {
      return ResponseHelper.error(res, 'CHAT_ALREADY_EXISTS', '이미 채팅방이 존재합니다', 409);
    }
    ...
    if (chatError) {
      ...
      throw new ApiError(500, '채팅방 생성 중 오류가 발생했습니다', 'CHAT_CREATION_ERROR');
    }
