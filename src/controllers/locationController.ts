...
export const locationController = {
      ...
      if (!region.parent_id) {
        ...
        if (parent) {
          ...
          if (parent.type === 'city') {
            ...
            // 시 하위의 구들 찾기
            const cityDistricts = regions?.filter(r => r.parent_id === region.id).map(r => r.name) || [];
  ...
  searchVenues: asyncHandler(async (req: Request, res: Response) => {
    ...
    // 지역 필터
    if (region) {
      query = query.or(`address.ilike.%${region}%,regions.name.ilike.%${region}%`);
    }
  ...
  getNearbyVenues: asyncHandler(async (req: Request, res: Response) => {
    const { lat, lng, radius = 10 } = req.query as { 
      lat?: string; 
      lng?: string; 
      radius?: string; 
    };
    ...
    if (!lat || !lng) {
      throw new ApiError(400, '위치 정보(위도, 경도)가 필요합니다', 'MISSING_LOCATION');
    }
    ...
    if (isNaN(latitude) || isNaN(longitude)) {
      throw new ApiError(400, '올바른 위치 정보를 입력해주세요', 'INVALID_COORDINATES');
    }
