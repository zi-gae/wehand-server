...
export const blockController = {
  ...
  blockUser: asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (userError || !targetUser) {
      logger.warn("Block attempt on non-existent user:", {
        blockerId,
        blockedId,
      });
  ...
  unblockUser: asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    return ResponseHelper.success(res, {
      message: `${
        targetUser?.nickname || targetUser?.name || "사용자"
      }님의 차단을 해제했습니다`,
    });
  }),
  ...
  // 특정 사용자와의 차단 상태 확인
  checkBlockStatus: asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // 양방향 차단 상태 확인
    const { data: blockStatus, error: statusError } = await supabase.rpc(
